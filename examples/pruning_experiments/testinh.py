...
filter_weights = torch.arange(0, 4, dtype=torch.double, device="cuda")
inputs = torch.arange(0, 3 * 3 * 3).reshape(3, 1, 3, 3)
...
for f_idx in range(filters_number):
    ...
    for i in range(patch_number):
        temp = torch.zeros(image_flattened_dim, dtype=torch.double, device=device)
