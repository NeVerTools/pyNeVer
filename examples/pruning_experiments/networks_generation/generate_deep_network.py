...
class SimpleNN(nn.Module):
    def __init__(self, input_dim, hdim, output_dim, n_hidden_layer):
        super(SimpleNN, self).__init__()

        layers = []
        # Primo strato (input -> primo livello nascosto)
        layers.append(nn.Linear(input_dim, hdim))
        layers.append(nn.ReLU())

        # Aggiungi gli strati nascosti (da 1 a n_hidden_layer)
        for _ in range(n_hidden_layer - 1):
            layers.append(nn.Linear(hdim, hdim))
            layers.append(nn.ReLU())

        # Strato finale (ultimo livello nascosto -> output)
        layers.append(nn.Linear(hdim, output_dim))

        # Definisci la sequenza come parte del modello
...
def create_batched_NN(input_dim, hdim, n_hidden_layer, output_dim):
    i = 1
    net = networks.SequentialNetwork("Net", "X")

    # Primo strato completamente connesso (input -> primo livello nascosto)
    ...
    # Aggiunta di strati nascosti (da 1 a n_hidden_layer)
    for _ in range(n_hidden_layer - 1):
        net.add_node(nodes.FullyConnectedNode(f'Linear_{i}', (hdim,), hdim))
        i += 1
        net.add_node(nodes.BatchNormNode(f'BatchNorm_{i}', (hdim,)))
        i += 1
        net.add_node(nodes.ReLUNode(f'ReLU_{i}', (hdim,)))
...
def generate_no_batch_networks(data_dict, hdim, n_hidden_layer):
    # Unpack data_dict
    ...
    model = SimpleNN(input_dim, hdim, output_dim, n_hidden_layer).to(device)
    ...
    print("Model Batch")
    small_net = create_batched_NN(input_dim, hdim, n_hidden_layer, output_dim)
