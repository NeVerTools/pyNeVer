            ...
            if isinstance(network, networks.SequentialNetwork):
                ...
                for layer in network.nodes.values():
                    ...
                    if isinstance(layer, nodes.ReLUNode):
                        ...
                        new_layer = tf_l.ELU(layer.identifier, layer.in_dim, layer.out_dim)

                    elif isinstance(layer, nodes.LeakyReLUNode):
                        new_layer = tf_l.LeakyReLU(layer.identifier, layer.in_dim, layer.out_dim)

                    elif isinstance(layer, nodes.SigmoidNode):
                        new_layer = tf_l.Sigmoid(layer.identifier, layer.in_dim, layer.out_dim)

                    elif isinstance(layer, nodes.TanhNode):
                        new_layer = tf_l.Tanh(layer.identifier, layer.in_dim, layer.out_dim)
    ...
    def to_neural_network(self, alt_rep: TensorflowNetwork) -> networks.NeuralNetwork:
        ...
        with tf.device('/cpu:0'):
            ...
            for m in alt_rep.tensorflow_network.submodules:
                ...
                elif isinstance(m, tf_l.ELU):
                    new_node = nodes.ELUNode(m.identifier, m.in_dim)

                if isinstance(m, tf_l.LeakyReLU):
                    new_node = nodes.LeakyReLUNode(m.identifier, m.in_dim)

                elif isinstance(m, tf_l.Sigmoid):
                    new_node = nodes.SigmoidNode(m.identifier, m.in_dim)

                if isinstance(m, tf_l.Tanh):
                    new_node = nodes.TanhNode(m.identifier, m.in_dim)
