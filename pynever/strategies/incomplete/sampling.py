...
def sample_property(my_property: verification.NeVerProperty, n_samples: int = 1000) -> list:
    """
    Sample random points from a set defined by a NeVer property

    Parameters
    ----------
    my_property : NeVerProperty
        The property of interest to sample
    n_samples : int
        The number of samples to compute (default 1000)
    ...
    bounds = PropertyFormatConverter(my_property).get_vectors()
    ...
    for p in sample_points:
        if any(np.greater(my_property.in_coef_mat @ p, my_property.in_bias_mat.flatten())):
...
def sampling(network: SequentialNetwork, property_path: str, n_points: int = 1000, subset_percentage: float = 0.1):
    ...
    sample_points = sample_property(prop, n_points)
